...
exports.applyForJob = async (req, res) => {
    try {
        // 1. Log params to your console to see exactly what key is being used
        console.log("URL Params:", req.params); 

        // 2. Extract Job ID (Check all common names to be safe)
        const jobId = req.params.id || req.params.jobId || req.params.jobID;
        const seekerId = req.user._id;

        // 3. Early check: Stop if we don't have a jobId
        if (!jobId) {
        ...
        // 4. Create using the exact schema keys
